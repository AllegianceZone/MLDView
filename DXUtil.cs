...
public class DXUtil
{
	...
	public static string SdkMediaPath
	{
		get
		{
			...
			Microsoft.Win32.RegistryKey rKey = Microsoft.Win32.Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Microsoft Games\Allegiance\1.0");

			string sReg = string.Empty;
			if (rKey != null)
			{
				//sReg = (string)rKey.GetValue(sdkKey);
				sReg = (string)rKey.GetValue("EXE Path");
				rKey.Close();
			}
			if (sReg != null)
				sReg += @"\Artwork\";
